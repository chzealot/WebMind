...
The `.claude/skills/` directory holds tool-like capabilities. **Claude Code auto-loads them via each `SKILL.md`'s frontmatter. Codex and other agents without a skill system have no auto-trigger — they MUST follow the table below**: when a task matches a "Trigger" below, read the linked `SKILL.md` and follow it before continuing. The table is the always-in-context index; each `SKILL.md` is the on-demand detail.

| Capability | Trigger | Details file |
|---|---|---|
| webmind-export | After any HTML file is created or modified and needs exporting to PDF/PNG | `.claude/skills/webmind-export/SKILL.md` |

For steps that are **mandatory every time** (such as export), the concrete command is also inlined below in "Export Guidelines" so no agent can miss it — do not rely on on-demand file reading alone for those.
...
Use the export tool at `.claude/skills/webmind-export/scripts/webmind-export`. A single command exports both PDF and PNG in one Chrome session. **Full parameters and build/download instructions live in `.claude/skills/webmind-export/SKILL.md`** — consult it if the binary is missing or you need advanced flags.

```bash
.claude/skills/webmind-export/scripts/webmind-export \
...
- Very tall pages (hundreds of cards) may be truncated by Chrome — pass `-scale 1` when exporting such pages.
