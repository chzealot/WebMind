...
每个 HTML 页面必须包含以下 `@media print` 样式，确保 Chrome headless 导出 PDF 时内容不被截断：

```css
@media print {
  /* 图片、表格、代码块、卡片：避免内部被截断到两页 */
  img, table, pre, code, figure, .card, blockquote {
    break-inside: avoid;
  }

  /* 标题不要孤立在页面底部（标题后必须跟内容） */
  h1, h2, h3, h4, h5, h6 {
    break-after: avoid;
  }
...
- `break-inside: avoid`：当元素放不下当前页剩余空间时，整体移到下一页，而非从中间截断
- `break-after: avoid`：防止标题出现在页面底部而正文在下一页的"孤标题"问题
- 这些规则由 Chrome 打印引擎原生支持，无需额外工具
